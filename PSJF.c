...
void add_proc_req()
{
for(rear=front;rear->next!=NULL;rear=rear->next); //this we have to
do because some time rear & curr on same position in the Q,when curr is
free then rear also free so to occupy rear's correct position this is
does.
for(i=1;i<=n;i++)
...
wtime[pid]+=(currtime-getctime(pid));//getctime() return

sespending time of pid,default arrival time.
...
proc[pid].atime=gchart[gindex].ctime+2;//2 is io
